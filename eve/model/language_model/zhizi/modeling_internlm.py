...
class InternLMMLPVanilla(nn.Module):
    def __init__(
        self,
        hidden_size: int,
        intermediate_size: int,
...
class FastGELUMindSpore(nn.Module):
    """
...
class InternLMDecoderLayerVanilla(nn.Module):
    def __init__(self, config: InternLMConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = InternLMAttention(config=config)
        self.mlp = InternLMMLPVanilla(
            hidden_size=self.hidden_size,
            intermediate_size=config.intermediate_size,
            hidden_act=config.hidden_act,
        )
...
class InternLMDecoderLayerAug(nn.Module):
    def __init__(self, config: InternLMConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = InternLMAttention(config=config)
    ...
    def forward(
        ...
        hidden_states = self.post_attention_layernorm(hidden_states)
        hidden_states = self.mlp(hidden_states) 
        # len(hidden_states) 16
        # hidden_states[0].shape torch.Size([171, 2048])
...
class InternLMModel(InternLMPreTrainedModel):
    ...
    def __init__(self, config: InternLMConfig):
        ...
        for idx in range(config.num_hidden_layers):
            if idx < split_dict[config.num_hidden_layers]:
                layer_list.append(InternLMDecoderLayerVanilla(config)) 
            else:
                layer_list.append(InternLMDecoderLayerAug(config)) 
