...
from .multimodal_encoder.builder import build_image_tower, build_audio_tower
from .multimodal_projector.builder import build_projector

from eve.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, \
    DEFAULT_IM_END_TOKEN, PAD_LENGTH, AUDIO_TOKEN_INDEX
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if getattr(config, "mm_image_tower", None) is not None or getattr(config, "mm_audio_tower", None) is not None:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        assert image_tower is not None or audio_tower is not None
        # ==============================================
        ...
        self.config.audio_projector_type = getattr(model_args, 'audio_projector_type', None)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_audios(self, audios, audios_len):  # [mini_b, c, t, h, w]
        audio_features = self.get_model().get_audio_tower()(audios, audios_len)  # [mini_b, t, n, c]
        audio_features = self.get_model().mm_projector.forward_audio(audio_features)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # if (image_tower is None and audio_tower is None) or images is None or input_ids.shape[1] == 1:
        #     if past_key_values is not None and (image_tower is not None or audio_tower is not None) and images is not None and input_ids.shape[1] == 1:
        #         # import ipdb
        #         # ipdb.set_trace()
        ...
        # dist.barrier()
            audios_idx = [idx for idx, audio in enumerate(audios)]


            # print(f'rank {dist.get_rank()}', 'image_idx', image_idx)
            # print(f'rank {dist.get_rank()}', 'video_idx', video_idx)
            audios_minibatch = torch.stack([audios[idx][0] for idx in audios_idx]) if len(audios_idx) > 0 else []  # mini_b c h w
            audios_minibatch_len = torch.stack([audios[idx][1] for idx in audios_idx]) if len(audios_idx) > 0 else []  # mini_b c h w


            tmp_audio_features = [None] * (len(audios_idx))
            if getattr(audios_minibatch, 'ndim', 0) == 3:  # batch consists of images, [mini_b, c, h, w]
                if audio_tower is not None:
                    # print(f'rank {dist.get_rank()}', 'image batch', images_minibatch.shape)
                    audio_features_minibatch = self.encode_audios(audios_minibatch, audios_minibatch_len)  # [mini_b, l, c]
                else:
                    audio_features_minibatch = torch.randn(1).to(self.device)  # dummy feature for video-only training under tuning
                for i, pos in enumerate(audios_idx):
                    tmp_audio_features[pos] = audio_features_minibatch[i]

            new_tmp = []
            for audio in tmp_audio_features:
                # print(len(new_tmp), len(image))
                if isinstance(audio, list):
                    t = len(audio)
                    for i in range(t):
                        new_tmp.append(audio[i])
                    # print('add video')
                else:
                    new_tmp.append(audio)
            audio_features = new_tmp
        ...
        if image_features is not None:
            ...
            for batch_idx, cur_input_ids in enumerate(input_ids):
                ...
                for i in range(num_images + 1):
                    ...
                    if i < num_images:
                        ...
                        cur_image_idx += 1
                        temp = torch.cat(cur_new_input_embeds)
                        start_imgidx = temp.shape[0]

                        end_imgidx = start_imgidx + 144
                        cur_new_input_embeds.append(cur_image_features)
                        imgidx_start_end = [start_imgidx, end_imgidx]
                        cur_new_labels.append(torch.full((cur_image_features.shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
                # import pdb;pdb.set_trace()
                # list_cur_new_input_embeds = cur_new_input_embeds
                ...
                list_imgidx_start_end.append(imgidx_start_end)
            ...
            if _position_ids is None:
                position_ids = None
            # import pdb;pdb.set_trace()
            return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels#, list_imgidx_start_end
        elif audio_features is not None:
            new_input_embeds = []
            new_labels = []
            list_auoidx_start_end = []
            cur_audio_idx = 0
            # print('for batch_idx, cur_input_ids in enumerate(input_ids)')
            # print('total:', sum([(i == IMAGE_TOKEN_INDEX).sum().item() for i in input_ids]), len(image_features), image_features[0].shape, image_features[-1].shape)
            for batch_idx, cur_input_ids in enumerate(input_ids):
                ...
                for i in range(len(audio_token_indices) - 1):
                    cur_input_ids_noauo.append(cur_input_ids[audio_token_indices[i]+1:audio_token_indices[i+1]])
                    cur_labels_noauo.append(cur_labels[audio_token_indices[i]+1:audio_token_indices[i+1]])
                split_sizes = [x.shape[0] for x in cur_labels_noauo]
                cur_input_embeds = self.get_model().embed_tokens(torch.cat(cur_input_ids_noauo))
                cur_input_embeds_no_auo = torch.split(cur_input_embeds, split_sizes, dim=0)
                ...
                cur_new_input_embeds = []
                cur_new_labels = []
                auoidx_start_end = [0,0]

                for i in range(num_audios + 1):
                    cur_new_input_embeds.append(cur_input_embeds_no_auo[i])
                    cur_new_labels.append(cur_labels_noauo[i])
                    if i < num_audios:
                        # print('cur_image_idx', cur_image_idx)
                        cur_audio_features = audio_features[cur_audio_idx].to(self.device)
                        cur_audio_idx += 1
                        temp = torch.cat(cur_new_input_embeds)
                        start_auoidx = temp.shape[0]

                        end_auoidx = start_auoidx + 144
                        cur_new_input_embeds.append(cur_audio_features)
                        auoidx_start_end = [start_auoidx, end_auoidx]
                ...
                new_input_embeds.append(cur_new_input_embeds)
                new_labels.append(cur_new_labels)
                list_auoidx_start_end.append(auoidx_start_end)
            ...
            if _position_ids is None:
                position_ids = None
            # import pdb;pdb.set_trace()
            return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels#, list_auoidx_start_end
        else:
