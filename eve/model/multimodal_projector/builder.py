...
from eve.model.multimodal_projector.pool_block import Pool_Block
from eve.model.multimodal_projector.qformer import qformer_config_template, Blip2Model, cheap_qformer_config_template, \
    Cheap_Blip2Model
from eve.model.multimodal_projector.simple_block import SimpleBlock, Cheap_SimpleBlock
from eve.model.multimodal_projector.ldp import LDPNetProjector, LDPNetV2Projector
...
def build_audio_projector(config, delay_load=False, **kwargs):
    ...
    if projector_type == 'identity':
        return IdentityMap()
    raise ValueError(f'Unknown audio projector type: {projector_type}')
...
class build_projector(nn.Module):
    def __init__(self, config, delay_load=False, **kwargs):
        ...
        mm_audio_tower = getattr(config, 'mm_audio_tower', None)
        # import pdb;pdb.set_trace()
        self.image_spatial_proj = build_image_projector(config, delay_load=False, **kwargs) if mm_image_tower is not None else None
        self.audio_spatial_proj = build_audio_projector(config, delay_load=False, **kwargs) if mm_audio_tower is not None else None
    ...
    def forward_audio(self, audio_feature):
        return self.audio_spatial_proj(audio_feature)
