...
from eve.mm_utils import tokenizer_image_token, process_images, get_model_name_from_path, KeywordsStoppingCriteria
...
def eval_model(args):
    ...
    if args.return_gating_logit is not None:
        from eve.utils import get_gating_logit_by_hook
        print(model)
    ...
    if args.return_gating_logit is not None:
        os.makedirs(os.path.dirname(args.return_gating_logit), exist_ok=True)
        torch.save(all_gating_logits, f'{args.return_gating_logit}')
