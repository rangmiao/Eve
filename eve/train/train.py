...
from eve.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
    ...
    MAX_VIDEO_LENGTH, DEFAULT_AUDIO_TOKEN, MAX_AUDIO_LENGTH, DEFAULT_AUDIO_START_TOKEN, DEFAULT_AUDIO_END_TOKEN
from torch.utils.data import Dataset
from eve.train.llava_trainer import LLaVATrainer

from eve import conversation as conversation_lib
from eve.model import *
from eve.mm_utils import tokenizer_image_token, tokenizer_audio_token
...
class ModelArguments:
    ...
    audio_tower_type: Optional[str] = field(default="whisper")
    ...
    moe_mode: str = field(
        ...
        metadata={
            ...
            "choices": ["first_half", "second_half", "sparse", "dense", "one_debug"],
        },
    )
    ...
    # =============================================================
    moe_fix_other: bool = field(default=False)
...
def preprocess_multimodal(
    ...
    for source in sources:
        for sentence in source:
            ...
            # ======================================================================================================
            if sentence['value'].startswith(DEFAULT_IMAGE_TOKEN) or sentence['value'].startswith(DEFAULT_AUDIO_TOKEN):  # run with multi-im, multi-vid, multi-im & multi-vid
            ...
            replace_token, audio_replace_token = DEFAULT_IMAGE_TOKEN, DEFAULT_AUDIO_TOKEN
...
def preprocess_v1(
    ...
    if has_image and not has_audio:
        ...
        raise("has_audio and has_image is not supported")
    else:
        input_ids = tokenizer(
            conversations,
            return_tensors="pt",
...
def preprocess(
    ...
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_2:
        return preprocess_llama_2(sources, tokenizer, has_image=has_image, has_audio=has_audio)
    if conversation_lib.default_conversation.version.startswith("phi") or \
        ...
        return preprocess_phi(sources, tokenizer, has_image=has_image, has_audio=has_audio)
    ...
    if conversation_lib.default_conversation.version.startswith("openchat") or \
        ...
        return preprocess_openchat(sources, tokenizer, has_image=has_image, has_audio=has_audio)
...
from eve.model.multimodal_encoder.whisper_tower_hg import load_audio, pad_or_trim, log_mel_spectrogram, get_T_after_cnn
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            if 'image' in sources[0] and 'audio' not in sources[0]:
                ...
                audio_file = order_pick_k(audio_file, MAX_IMAGE_LENGTH) # TODO

                audio = [audio_preprocess(os.path.join(audio_folder, file)) for file in audio_file]
                sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
                data_dict = preprocess(sources, self.tokenizer, has_image=False, has_audio=True)
            elif 'audio' in sources[0] and 'image' in sources[0]:
                raise("is not supported")
            else:
                sources = copy.deepcopy([e["conversations"] for e in sources])
                data_dict = preprocess(sources, self.tokenizer, has_image=False, has_audio=False)
...
def train():
    ...
    if model_args.image_tower is not None or model_args.video_tower is not None:
        if not model_args.moe_enable:
            ...
            elif 'zhizi' in model_args.model_name_or_path.lower():
                model = LlavaZhiziForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    **bnb_model_from_pretrained_args)
            else:
            ...
            if 'qwen' in model_args.model_name_or_path.lower() and '1.5' not in model_args.model_name_or_path.lower():
                model = EveLLaVAQWenForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    **bnb_model_from_pretrained_args
                )
            elif 'qwen' in model_args.model_name_or_path.lower() and '1.5' in model_args.model_name_or_path.lower():
                model = EveLLaVAQwen1_5ForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    # attn_implementation="flash_attention_2",
                    # torch_dtype=torch.bfloat16,
                ...
                model = EveLLaVAPhiForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    # attn_implementation="flash_attention_2",
                    # torch_dtype=torch.bfloat16,
                ...
                model = EveLLaVAMiniCPMForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    # attn_implementation="flash_attention_2",
                    # torch_dtype=torch.bfloat16,
                ...
                model = EveLLaVAMistralForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    # attn_implementation="flash_attention_2",
                    # torch_dtype=torch.bfloat16,
                ...
                model = EveLLaVAStablelmForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    # attn_implementation="flash_attention_2",
                    # torch_dtype=torch.bfloat16,
                ...
                model = EveLlavaZhiziForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    **bnb_model_from_pretrained_args
                )
            else:
                model = EveLLaVALlamaForCausalLM.from_pretrained(
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    attn_implementation="flash_attention_2",
                    torch_dtype=torch.bfloat16,
    ...
    if model_args.moe_enable:
        if training_args.lora_enable:
            ...
            if 'qwen' in model_args.model_name_or_path.lower() and '1.5' not in model_args.model_name_or_path.lower():
                ...
                target_modules = [
                    'gate_proj', 'gate_proj2', 'down_proj', 'up_proj'
                ] if training_args.only_lora_ffn else find_all_linear_names(model)
            else:
                target_modules = [
                    'up_proj', 'down_proj', 'gate_proj'
                ] if training_args.only_lora_ffn else find_all_linear_names(model)
            # modules_to_save = ['wg']  # weight gating for Eve
    ...
    # =============================================================================================================
    if model_args.image_tower is not None or model_args.audio_tower is not None:
        # print(model_args)
    ...
    if model_args.moe_enable and torch.cuda.device_count() == 1:
        print("freeze other layer for moe")
